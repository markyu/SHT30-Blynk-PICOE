...
BLYNK_TEMPLATE_ID =  "12312312312"
BLYNK_DEVICE_NAME = "12312312312312"
BLYNK_AUTH_TOKEN = "12312312312312312"
...
# Maintain DHCP lease (continue the DHCP setting while TCP is connected)
    ...
    if client.status == SNSR_SOCK_ESTABLISHED:
        blynk.run()
        blynk.virtual_write(4, str(sensor.temperature))
        blynk.virtual_write(6, str(sensor.relative_humidity))
        print ("Temp: {temp}, Humidity: {humid}".format(temp = str(sensor.temperature),
                                                        humid = str(sensor.relative_humidity))
               )
        time.sleep(1)
        counter +=1
